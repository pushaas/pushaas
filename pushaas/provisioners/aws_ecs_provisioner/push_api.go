...
func createPushApiTaskDefinition(
	...
	instanceName := pushApiWithInstance(instance.Name)

	return ecsSvc.RegisterTaskDefinition(&ecs.RegisterTaskDefinitionInput{
		Family: aws.String(instanceName),
				...
				Environment: []*ecs.KeyValuePair{
					{
						...
						Value: aws.String(fmt.Sprintf("redis://%s.tsuru:6379", instanceName)),
					},
					{
						Name: aws.String("PUSHAPI_PUSH_STREAM__URL"),
						Value: aws.String(fmt.Sprintf("http://%s.tsuru:9080", instanceName)),
					},
				},
			},
		},
...
func createPushApiService(
	...
	input := &ecs.CreateServiceInput{
		...
		TaskDefinition: aws.String(instanceName),
	...
	return ecsSvc.CreateService(input)
}
